...
export const GstInvoicePreview = forwardRef<HTMLDivElement, GstInvoicePreviewProps>(({ bill, settings }, ref) => {
  ...
  const customerGstin = gstField(bill, ["Customer GSTIN"]);
  ...
  return (
    <div className="print-receipt flex w-full justify-center overflow-x-auto bg-slate-200 p-4 md:p-8 print:bg-white print:p-0">
      <div ref={ref} className="min-w-[760px] max-w-[900px] bg-white p-8 text-slate-800 shadow-lg print:shadow-none md:p-12">
        <div className="border-b-2 border-slate-800 pb-5 text-center">
          ...
          <p className="text-sm text-slate-600">Phone: {settings.phone}{settings.email ? ` | ${settings.email}` : ""}</p>
        ...
        <div className="grid grid-cols-2 gap-6 border-b border-slate-300 py-5 text-sm">
          <div>
            ...
            {customerGstin && <p className="mt-1 font-medium">GSTIN: {customerGstin}</p>}
          ...
          <div className="text-right">
            ...
            <p className="mt-1"><span className="font-semibold">Reverse Charge:</span> {gstField(bill, ["Reverse Charge"]) || "No"}</p>
          </div>
        </div>
        ...
        <div className="mt-12 flex justify-between border-t border-slate-300 pt-5 text-sm">
          <div className="max-w-md text-slate-500">
            <p className="font-semibold text-slate-700">Declaration</p>
            <p className="mt-1">This is a GST-style tax invoice generated by the billing application. It is not an official e-invoice and does not contain an IRN or government verification.</p>
          </div>
          <div className="w-48 text-center">
            <div className="h-10 border-b border-slate-400"></div>
            <p className="mt-2 font-semibold">Authorized Signatory</p>
            <p className="text-slate-500">{settings.authorizedSignatory || settings.ownerName}</p>
          </div>
        </div>
      </div>
    </div>
